...
// 环境变量
const UUID = process.env.UUID || '131d842b-1b6a-4d3b-a5ed-ea1872e3b5cc'; // 使用哪吒v1，在不同的平台部署需修改UUID，否则会覆盖
...
const NEZHA_KEY = process.env.NEZHA_KEY || 'JFPqIyPYAKhI7GcECQ3XbPxONPE1MYHl';             // v1的NZ_CLIENT_SECRET或v0的agent端口  
...
const SUB_PATH = process.env.SUB_PATH || 'wubp';            // 节点订阅路径
const DOMAIN = process.env.DOMAIN || '0e3wru6ps9o5-production-v4rok3km.australia-southeast1.suga.run';                   // 域名或ip,留空将自动获取服务器ip
...
const getDownloadUrl = () => {
    ...
    if (arch === 'arm' || arch === 'arm64' || arch === 'aarch64') {
      if (!NEZHA_PORT) {
        return 'https://arm64.00666.xyz/v1';
      } else {
          return 'https://arm64.00666.xyz/agent';
      }
    } else {
      if (!NEZHA_PORT) {
        return 'https://amd64.00666.xyz/v1';
      } else {
          return 'https://amd64.00666.xyz/agent';
      }
    }
};
